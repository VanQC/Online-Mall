...
// 序列化收藏夹
func BuildFavorite(item1 *model.Favorite, item2 *model.Product, item3 *model.Member) Favorite {
	return Favorite{
		UserID:        item1.MemberID,
...
func BuildFavorites(ctx context.Context, items []*model.Favorite) (favorites []Favorite) {
	...
	for _, fav := range items {
		...
		boss, err := bossDao.QueryById(fav.MemberID)
		if err != nil {
			continue
		}
